...
export default function ContractScreen({ navigation, route }) {
  ...
  useEffect(() => {
    const fetchArriendo = async () => {
      try {
        ...
        console.log("📌 DATOS ARRIENDO:", response.data);

        setArriendo(response.data.data);
  ...
  const downloadTemplate = () => {
    const url = "http://192.168.1.3:3000/api/arriendos/template";
  ...
  const uploadContract = async () => {
    try {
      ...
      if (Platform.OS === "web") {
        ...
        formData.append("contrato", blob, file.name);
      } else {
        // 📌 Para Android / iOS
      ...
      // Recargar arriendo actualizado
      const updated = await axios.get(`${API_BASE}/api/arriendos/${id}`);
      setArriendo(updated.data.data);

      Alert.alert("Éxito", "Contrato subido correctamente.");
    } catch (err) {
      Alert.alert("Error", "No se pudo subir el archivo.");
      console.log("ERR SUBIDA:", err);
    } finally {
      setUploading(false);
    }
  };
  ...
  const viewContract = () => {
    if (!arriendo?.contrato_archivo)
      return Alert.alert("Sin archivo", "No hay acontrato subido.");

    const url = arriendo?.contrato_archivo?.startsWith("http")
      ? arriendo.contrato_archivo
      : `${API_BASE}/uploads/contratos/${arriendo.contrato_archivo}`;
  ...
  const sendEmail = () => {
    ...
    let link = "Contrato aún no disponible";

    if (arriendo?.contrato_archivo) {
      link = arriendo.contrato_archivo.startsWith("http")
        ? arriendo.contrato_archivo
        : `${API_BASE}/uploads/contratos/${arriendo.contrato_archivo}`;
    }
  ...
  console.log("🔎 ID RECIBIDO EN ContractScreen:", id);
              ...
              {arriendo && (
                <>
                  <View style={{ marginTop: 15 }}>
                    ...
                    <Text>
                      {arriendo.estudiante_nombres} {arriendo.estudiante_apellidos}
                    </Text>
