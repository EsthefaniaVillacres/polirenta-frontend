...
const EditRoomScreen = ({ navigation, route }) => {
  ...
  const allowedCoexistence = useMemo(
    () => convivencia_residencia || [],
    [convivencia_residencia]
  );
  ...
  const roomId = roomData?.id_habitacion;
  ...
  }, [roomData, allowedAmenities, allowedCoexistence]);
  ...
  const handleEditRoom = async () => {
    try {
      ...
      const response = await fetch(
        `http://192.168.1.3:3000/api/auth/rooms/${roomId}`,
        {
          method: "PUT",
          body: formData,
        }
