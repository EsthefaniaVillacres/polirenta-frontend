...
const RentalViewScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const checkRentalStatus = async () => {
      try {
        const response = await axios.get(
          "http://192.168.1.3:3000/api/solicitudes/check",
          {
            params: {
    ...
    if (user?.id && route.params?.departamentoId) {
      checkRentalStatus();
    }
  }, [user?.id, route.params?.departamentoId]);
  ...
  const handleRentalRequest = async () => {
    ...
    try {
      ...
      const requestResponse = await axios.post(
        "http://192.168.1.3:3000/api/solicitudes",
        {
          id_estudiante: user.id,
          id_propiedad: route.params.departamentoId,
        }
      ...
      if (requestResponse.status === 201) {
        ...
        const propertyResponse = await axios.get(
          `http://192.168.1.3:3000/api/auth/properties/${route.params.departamentoId}`
        );
      ...
      if (error.response?.status === 409) {
        alert("Ya tienes una solicitud pendiente para esta propiedad.");
  ...
  const isButtonDisabled = () => {
    return isLoading || rentalStatus !== null;
  };
...
export default RentalViewScreen;
